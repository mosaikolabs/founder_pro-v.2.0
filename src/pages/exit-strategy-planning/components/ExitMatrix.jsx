...
const ExitMatrix = ({ activities, onActivityClick }) => {
  const exitAreas = [
    'exit-strategy', 
  ...
  const getCellColor = (completed, total) => {
    ...
    if (percentage >= 50) return 'bg-gradient-to-br from-violet-500/40 to-violet-400/40 border-violet-500/50';
  ...
  const getAreaIcon = (area) => {
    const icons = {
      ...
      'valuation-optimization': 'TrendingUp',
      'acquisition-preparation': 'Building',
      'ipo-readiness': 'DollarSign',
      'stakeholder-management': 'Users'
    };
    return icons[area] || 'Target';
  };
  ...
  const getAreaDisplayName = (area) => {
    const displayNames = {
      ...
      'acquisition-preparation': 'Acquisition',
      'ipo-readiness': 'IPO',
  ...
  const getAreaDescription = (area) => {
    const descriptions = {
      'exit-strategy': 'Estrategia salida y timing',
      'valuation-optimization': 'Optimización valuation máxima',
      'acquisition-preparation': 'Preparación acquisition ready',
      'ipo-readiness': 'IPO readiness y compliance',
      'stakeholder-management': 'Gestión stakeholders y alineación'
    };
    return descriptions[area] || 'Área de exit';
  };
  ...
  return (
    <div className="bg-gradient-card rounded-neumorphic-lg p-6 shadow-neumorphic border border-slate-600">
      <h3 className="text-2xl font-bold text-text-dark-primary mb-6 flex items-center">
        <Icon name="Target" size={24} className="mr-3 text-violet-500" />
        Matriz Exit Strategy Planning
      </h3>
      
      <p className="text-text-dark-secondary mb-6">
        Planificación estratégica de exit, maximización de valuation y preparación para acquisition o IPO.
      </p>
                  ...
                  <Icon 
                    ...
                    className={`
                      ${phase.status === 'completed' ? 'text-neon-green' :
                        phase.status === 'in-progress'? 'text-violet-500' : 'text-warning-dark'
                      }
                    `} 
                  />
                </div>
            ...
            <div key={area} className="grid grid-cols-6 gap-2 mb-2">
              ...
              <div className="p-4 bg-dark-primary rounded-neumorphic shadow-neumorphic-subtle border border-slate-700 flex items-center">
                <Icon 
                  ...
                  size={16} 
                  className="text-violet-500 mr-3 flex-shrink-0" 
                />
                  ...
                  >
                    {total > 0 ? (
                      <div className="text-center">
                        <div className="text-lg font-bold text-text-dark-primary group-hover:text-violet-500 transition-colors duration-300">
                          {completed}/{total}
                        </div>
                        ...
                        <div className="w-full bg-dark-surface/50 rounded-full h-1 mt-2">
                          <div 
                            className="bg-gradient-to-r from-violet-500 to-violet-400 h-1 rounded-full transition-all duration-300"
      ...
      <div className="mt-6 flex items-center justify-center space-x-6 text-sm">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-3 h-3 rounded bg-gradient-to-br from-violet-500/40 to-violet-400/40 border border-violet-500/50"></div>
            ...
            return (
              <div key={area} className="bg-dark-primary rounded-neumorphic p-3 border border-slate-700">
                <div className="flex items-center mb-2">
                  <Icon name={getAreaIcon(area)} size={14} className="text-violet-500 mr-2" />
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-medium ${
                    ...
                    progressPercentage >= 50 ? 'text-violet-500' :
                ...
                <div className="w-full bg-dark-surface/50 rounded-full h-1 mt-2">
                  <div 
                    className={`h-1 rounded-full transition-all duration-300 ${
                      ...
                      progressPercentage >= 50 ? 'bg-gradient-to-r from-violet-500 to-violet-400' :
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-violet-500/10 to-violet-400/10 border border-violet-500/30 rounded-neumorphic">
        <div className="flex items-start space-x-3">
          <Icon name="Info" size={20} className="text-violet-500 mt-0.5" />
          <div>
            <h5 className="font-semibold text-text-dark-primary mb-1">
              Excelencia en Exit Strategy
            </h5>
            <p className="text-sm text-text-dark-secondary">
              Planifica estratégicamente tu salida para maximizar valuation, preparar acquisition readiness 
              y optimizar el timing perfecto para IPO o venta estratégica.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-dark-primary rounded-neumorphic p-3 border border-slate-700 text-center">
          <Icon name="Target" size={20} className="text-violet-500 mx-auto mb-2" />
          <div className="text-sm font-medium text-text-dark-primary">Exit $100M+</div>
          <div className="text-xs text-text-dark-muted">valuation target</div>
        </div>
        
        <div className="bg-dark-primary rounded-neumorphic p-3 border border-slate-700 text-center">
          <Icon name="Building" size={20} className="text-violet-500 mx-auto mb-2" />
          <div className="text-sm font-medium text-text-dark-primary">Tier 1 Ready</div>
          <div className="text-xs text-text-dark-muted">acquirer status</div>
        </div>
        
        <div className="bg-dark-primary rounded-neumorphic p-3 border border-slate-700 text-center">
          <Icon name="DollarSign" size={20} className="text-violet-500 mx-auto mb-2" />
          <div className="text-sm font-medium text-text-dark-primary">IPO Ready</div>
          <div className="text-xs text-text-dark-muted">compliance</div>
        </div>
        
        <div className="bg-dark-primary rounded-neumorphic p-3 border border-slate-700 text-center">
          <Icon name="Users" size={20} className="text-violet-500 mx-auto mb-2" />
          <div className="text-sm font-medium text-text-dark-primary">100% Aligned</div>
          <div className="text-xs text-text-dark-muted">stakeholders</div>
        </div>
      </div>

      {/* Exit Excellence Framework */}
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-violet-500/10 border border-violet-500/30 rounded-neumorphic">
          ...
          <ul className="space-y-1 text-xs text-text-dark-secondary">
            <li>• Exit strategy timing optimal</li>
            <li>• Valuation optimization maximizada</li>
            <li>• Acquisition readiness completa</li>
            <li>• IPO compliance y preparación</li>
          </ul>
        </div>
        ...
        <div className="p-4 bg-warning-dark/10 border border-warning-dark/30 rounded-neumorphic">
          ...
          <ul className="space-y-1 text-xs text-text-dark-secondary">
            <li>• Valuation por debajo de potencial</li>
            <li>• Acquisition readiness incompleta</li>
            <li>• IPO compliance gaps críticos</li>
            <li>• Stakeholder misalignment riesgoso</li>
          </ul>
        </div>
      </div>
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-violet-500/10 to-violet-400/10 border border-violet-500/30 rounded-neumorphic">
        <h5 className="font-semibold text-text-dark-primary mb-3 flex items-center">
          <Icon name="DollarSign" size={18} className="mr-2 text-violet-500" />
          Métricas Clave de Exit
        </h5>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-text-dark-muted">Compliance Ready</div>
          </div>
          <div className="text-center">
