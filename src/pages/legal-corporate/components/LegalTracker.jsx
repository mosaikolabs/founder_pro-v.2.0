...
const LegalTracker = ({ milestones = [] }) => {
  const getMilestoneIcon = (milestoneId) => {
    const icons = {
      'company-legally-established': 'Building',
      'ip-protection-complete': 'Copyright',
      'compliance-framework-ready': 'Shield',
      'legal-scaling-prepared': 'Scale'
    };
    return icons[milestoneId] || 'FileText';
  };
  ...
  const getLegalPriority = (milestoneId) => {
    const priorities = {
      'company-legally-established': 'Alta',
      'ip-protection-complete': 'Media',
      'compliance-framework-ready': 'Alta',
      'legal-scaling-prepared': 'Media'
    };
    return priorities[milestoneId] || 'Media';
  };
    ...
    return (
      <div className="bg-gradient-card rounded-neumorphic-lg p-6 shadow-neumorphic border border-slate-600">
        <h3 className="text-2xl font-bold text-text-dark-primary mb-6 flex items-center">
          <Icon name="FileText" size={24} className="mr-3 text-neon-purple" />
          Seguimiento de Milestones Legales
        </h3>
        
        <div className="text-center py-12">
          <Icon name="Scale" size={48} className="text-text-dark-muted mb-4 mx-auto" />
          <h4 className="text-lg font-semibold text-text-dark-primary mb-2">
            No hay milestones legales definidos
          </h4>
          <p className="text-text-dark-secondary">
            Los milestones legales aparecerán aquí una vez configurados
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-gradient-card rounded-neumorphic-lg p-6 shadow-neumorphic border border-slate-600">
      ...
      <p className="text-text-dark-secondary mb-6">
        Monitorea el progreso hacia los hitos legales y corporativos críticos para el cumplimiento de tu startup
      </p>
      ...
      <div className="space-y-6">
        {milestones.map((milestone) => {
          ...
          const priority = getLegalPriority(milestone.id);
            ...
            >
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 mb-3">
                      ...
                      <div className="px-2 py-1 rounded-full text-xs font-medium bg-neon-purple/20 text-neon-purple">
                        Prioridad {priority}
                      </div>
                    </div>
                    
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-text-dark-secondary">Progreso Legal</span>
              ...
              <div className="mt-4 pt-4 border-t border-slate-600">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    <span className="text-text-dark-muted">Estado Legal:</span>
                    <span className={`ml-2 font-medium ${getMilestoneStatusColor(milestone.progress)}`}>
                      {milestone.progress >= 100 ? 'Conforme' :
                       milestone.progress >= 75 ? 'Casi Conforme' :
                       milestone.progress >= 50 ? 'En Proceso' :
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium text-neon-purple">
                      Legal & Corporativo
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-3 p-3 bg-neon-purple/10 border border-neon-purple/30 rounded-neumorphic">
                  <div className="flex items-start space-x-2">
                    <Icon name="Info" size={16} className="text-neon-purple mt-0.5" />
                    <div className="text-xs text-text-dark-secondary">
                      <strong className="text-neon-purple">Recomendación Legal:</strong> 
                      {milestone.id === 'company-legally-established' && " Asegúrate de completar todos los registros corporativos antes de operaciones comerciales."}
                      {milestone.id === 'ip-protection-complete' && " Protege tu propiedad intelectual antes de divulgar información sensible."}
                      {milestone.id === 'compliance-framework-ready' && " Mantén actualizadas todas las políticas de cumplimiento regulatorio."}
                      {milestone.id === 'legal-scaling-prepared' && " Prepara la documentación legal necesaria para el crecimiento internacional."}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="mt-8 pt-6 border-t border-slate-600">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-dark-primary rounded-neumorphic p-4 border border-slate-700 text-center">
            ...
            <div className="text-xs text-text-dark-muted">Conformes</div>
          </div>
          ...
          <div className="bg-dark-primary rounded-neumorphic p-4 border border-slate-700 text-center">
            ...
            <div className="text-xs text-text-dark-muted">En Proceso</div>
          </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-text-dark-secondary">Cumplimiento General</span>
          ...
          <div className="w-full bg-dark-surface/50 rounded-full h-2">
            <div 
              className="bg-gradient-to-r from-neon-purple to-purple-400 h-2 rounded-full transition-all duration-500"
